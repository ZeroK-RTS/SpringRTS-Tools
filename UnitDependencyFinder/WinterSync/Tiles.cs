...
namespace MapReader
{
    static unsafe class Tiles
    {
        ...
        public static Bitmap LoadTiles(Dictionary<byte[], int> tileFiles, int[] tilePositions, int tilesX, int tilesY, int mipLevel)
        {
            ...
            var map = new Bitmap(bitmapWidth, bitmapHeight, PixelFormat.Format32bppRgb);

            // Expose its data.
            var bitmapData = map.LockBits(new Rectangle(Point.Empty, map.Size), ImageLockMode.ReadWrite, PixelFormat.Format32bppRgb);
            var decompressedData = (byte*) bitmapData.Scan0;
            ...
            foreach (var kvp in tileFiles) {
                ...
                GCHandle tileFileHandle = GCHandle.Alloc(tileFileBuffer, GCHandleType.Pinned);
                ...
                Action<int> processTile = tileInTileFileIndex => {
                      ...
                      int firstUseOffsetX = 0;
                      int firstUseOffsetY = 0;

                      for (var tileUseIndex = 0; tileUseIndex < tiles.Count; tileUseIndex++) {
                          byte* compressedData = tileFileData + tileInTileFileIndex*TileTotalBytes + mipLevelOffset;
        ...
        static void CopyBitmap32Section(byte* bitmap, int tileSize, int stride, int sourceOffsetX, int sourceOffsetY, int destOffsetX, int destOffsetY)
        {
        ...
        public static void DecodeDxt1Texture(byte* sourceBitmap, int size, byte* destinationBitmap, int stride, int offsetX, int offsetY)
        {
            int blockCount = ((size + 3)/4)*((size + 3)/4);
            byte* progress = sourceBitmap;
        ...
        static void DecodeDxt1Block(byte* blockData, int* destinationBitmap, int offsetX, int offsetY, int stride)
        {
            ...
            int r0 = UnpackRed(colors[0]);
            int g0 = UnpackGreen(colors[0]);
            int b0 = UnpackBlue(colors[0]);

            int r1 = UnpackRed(colors[3]);
            int g1 = UnpackGreen(colors[3]);
            int b1 = UnpackBlue(colors[3]);

#if false
           This is the correct code for a 32bit BGR bitmap:
           ...
           the following section, written for 32bit RGB, is probably wrong. 
#endif
            
            colors[2] = r0 << 16 | g0 << 8 | b0;
            colors[1] = r1 << 16 | g1 << 8 | b1;
            colors[0] = ((r0*2 + r1)/3) << 16 | ((g0*2 + g1)/3) << 8 | ((b0*2 + b1)/3);
            colors[3] = ((r0 + r1*2)/3) << 16 | ((g0 + g1*2)/3) << 8 | ((b0 + b1*2)/3);

            uint bits = *(uint*) &blockData[4];

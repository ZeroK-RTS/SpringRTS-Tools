...
local staticTexNames = {[1] = {[1] = "LuaUI/Images/test.png", [2] = false}, }
...
function widget:Initialize()
	if (not staticTextures) then
		...
		for i = 1, (numTexSquaresX * numTexSquaresZ) do
			squareTextures[i] = glCreateTexture(texSQS, texSQS, {wrap_s = GL.CLAMP_TO_EDGE, wrap_t = GL.CLAMP_TO_EDGE})
		...
		if (SpringGetGameFrame() ~= lastGameFrame) then
			...
			if (mmodulo(lastGameFrame, Game.gameSpeed) == 0) then
				for i = 1, #squareTextures do
					...
					texSquareX = mmodulo((i - 1), numTexSquaresX)
					texSquareZ = mfloor((i - 1) / numTexSquaresX)
					texSquareN = i-1
